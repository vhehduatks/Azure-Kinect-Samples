@echo off
REM Copy Orbbec K4A Wrapper DLLs and OpenCV to build output directory
...
echo.
echo === Orbbec DLL Copy Script for egocentric_dataset_generator ===
echo.
...
if not exist "%OPENCV_BIN%" (
    echo ERROR: OpenCV not found at %OPENCV_BIN%
    echo Please update the OPENCV_BIN path in this script.
    exit /b 1
)
...
echo From: %OPENCV_BIN%
copy /Y "%OPENCV_BIN%\opencv_world4120.dll" "%BUILD_DIR%\"
